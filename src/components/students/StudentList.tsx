...
export default function StudentList({buttonTitle="Add", buttonColor="bg-green-600", searchBar=true, id, context, emptyMessage}: StudentListProps) {
    const [selectedStudent, setSelectedStudent] = useState<StudentRecord | null>(null)
    const [isAssigningStudentToGuardian, setIsAssigningStudentToGuardian] = useState(false)
    const [isAssiginingStudentToClass, setIsAssigningStudentToClass] = useState(false)
    const [isRemovingStudentFromClass, setIsRemovingStudentFromClass]= useState(false)
    ...
    function handleMethod(student: StudentRecord){
        ...
        switch(context){
            case "guardian": return setIsAssigningStudentToGuardian(true)
            case "enrollment-assignable": return setIsAssigningStudentToClass(true)
            case "enrollment-assigned": return setIsRemovingStudentFromClass(true)
            }
        }
    

    const {data=[], isLoading, isError, error} = useQuery<StudentRecord[]>({
        queryKey: getQueryKey(),
        queryFn: ()=>{
            if(!id || !context) return fetchStudents()
            switch(context){
                case "guardian": return getAvailableGuardianStudents(id)
                case "enrollment-assignable": return getClassAvailableStudents(id)     
                case "enrollment-assigned": return getClassStudents(id)     
                default: return fetchStudents()
            }
        }})
        ...
        <div>
            ...
            {isLoading ? <LoadingSpinner/>:
                data.length !== 0 ? (
                    ...
                    searchBar={searchBar}
                    maxHeight="max-h-100"
                    addButtonOnClick={showActionButton ? ((row) => handleMethod(row)): undefined}
