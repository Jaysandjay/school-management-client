...
export default function StudentsGradesList({id}:StudentsGradesListProps){
    ...
    const {data=[], isLoading, isError, error} = useQuery<StudentGrade[]>({
    ...
    const saveGradesMutation = useMutation({
        mutationFn: (updates: { studentId: number; grade: number }[]) =>
            Promise.all(updates.map(u => updateStudentGrade(u.studentId, id, u.grade))),
        onSuccess: () => {
    ...
    function handleChangedgrade(e: ChangeEvent<HTMLInputElement>, student: StudentGrade) {
    ...
    return(
        ...
        <div className="min-h-0 flex flex-col h-full flex-1">
                ...
                <div className={`max-h-110 min-h-0 flex-1 overflow-y-auto rounded-lg border border-slate-200 bg-white shadow-sm`}>
                                ...
                                <tr key={student.studentId} className="hover:bg-slate-50">
                                    ...
                                    {isEditing ? (
                                        <td>
                                            ...
                                            max={100}
                                            step={1}
                                            value={editedGrades[student.studentId] ?? student.grade}
                ...
                <div className="mt-5 flex justify-end">
                    <div className="self-end flex gap-1">
                        {isEditing? (
                            ...
                            />
                            <PrimaryButton title="cancel" color="bg-red-600" onclick={()=>handleCancel()}/>        
                            </>
                        ):(

                    <PrimaryButton title="Edit" onclick={()=>setIsEdiiting(true)}/>
                        )}
                    </div>
                </div>
            </div>
