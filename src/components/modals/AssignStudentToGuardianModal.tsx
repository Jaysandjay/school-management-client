...
export default function AssignStudentToGuardianModal ({guardianId, student, isOpen, onClose}: AssignStudentToGuardianProps) {
    ...
    return (
        <BasicModalContainer>
            {isSuccessfullyAssigned ? (
                <h2>{student.firstName} {student.lastName} has been assigned</h2>
            ):(
                <FormDropDownInput
                    label="Relationship"
                    name="relationship"
            ...
            <div>
                ...
                />
                {!isSuccessfullyAssigned && 
                <PrimaryButton title="Assign" color="bg-green-500" onclick={()=>assignStudent()}/>
                }
            </div>
        </BasicModalContainer>
    )
