...
export default function UnenrollStudentModal ({type, classId, studentId, isOpen, onClose}: UnenrollStudentModalProps) {
    ...
    mutationFn: ({studentId, classId}: UnenrollVariables) => unenrollStudent(studentId, classId),
    onSuccess: () => {
        switch(type){
            case "student": {
                queryClient.invalidateQueries({queryKey: ["student-classes", studentId, "enrolled"]}),
    ...
    return (
        <BasicModalContainer >
            ...
            {isSuccessfullyRemoved ? (
                ...
                <h2>Are you sure you want to unenroll {student?.firstName} {student?.lastName} from {course.className}</h2>
            )}
