...
export default function Table<RowType extends object>({searchBar=true, maxHeight, checked, addButtonColor="bg-green-600", addButtonTitle, addButtonOnClick, columns, rows, urls, idField }: TableProps<RowType>){
    ...
    return (
        ...
        <div className={`${maxHeight && maxHeight}  min-h-0 flex-1 overflow-y-auto rounded-lg border border-slate-200 bg-white shadow-sm`}>
        <table className="min-w-full border-collapse flex-1">
            ...
            <tr>
                ...
                {urls && <th className="px-4 py-3 text-left text-sm font-semibold text-slate-700">More</th>}
            </tr>
            </thead>
            ...
            <tbody className="divide-y divide-slate-200">
                ...
                <td className="px-4 py-3">
                  <div className="flex justify-between ">
                    ...
                    {addButtonOnClick && addButtonTitle && <PrimaryButton color={addButtonColor} title={addButtonTitle} onclick={() => addButtonOnClick(row)}/>}
                  </div>
                </td>
                )}
              </tr>
