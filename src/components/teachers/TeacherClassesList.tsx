...
export default function TeacherClassesList({id,  emptyMessage, context}: TeacherClassesListProps){
    ...
    const {data: classes = [], isLoading, isError, error} = useQuery<CourseRecord[]>({
        queryKey:  context === "assigned" ? ["teacher-classes", id, context] : ["classes", "unassigned"],
        queryFn: ()=>
            context === "assigned" ?
        ...
        {isError && <p>Error..</p>}
            {classes.length !== 0? (
                ...
                addButtonOnClick={(row) => {
                    ...
                    context === "assigned" ? setIsRemovingClass(true) : setIsAddingClass(true)

                }}
                addButtonColor={context === "assigned" ? "bg-red-600": "bg-green-600"}
